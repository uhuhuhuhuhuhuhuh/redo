...
/**
 * A deliberately package-scoped relay. It never reads authentication data or network traffic. It
 * inserts a user-requested prompt into the visible ChatGPT UI and speaks the longest new visible
 * answer after the screen has remained stable.
 */
public final class ChatRelayAccessibilityService extends AccessibilityService {
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        if (event.getPackageName() == null || !CHATGPT_PACKAGE.contentEquals(event.getPackageName())) {
            return;
        }
        AccessibilityNodeInfo root = getRootInActiveWindow();
        if (root == null) return;

        String pending = getSharedPreferences(ChatGptLauncher.PREFS, MODE_PRIVATE)
                .getString(ChatGptLauncher.KEY_PENDING_PROMPT, null);
        ...
        if (pending != null && !pending.trim().isEmpty() && !injectionInProgress) {
            ...
            if (inserted) {
                ...
                getSharedPreferences(ChatGptLauncher.PREFS, MODE_PRIVATE)
                        .edit().remove(ChatGptLauncher.KEY_PENDING_PROMPT).apply();
            }
    ...
    private AccessibilityNodeInfo findEditable(AccessibilityNodeInfo node) {
        ...
        if (node.isEditable() || EditText.class.getName().contentEquals(node.getClassName())) return node;
        for (int index = node.getChildCount() - 1; index >= 0; index--) {
            AccessibilityNodeInfo found = findEditable(node.getChild(index));
    ...
    private AccessibilityNodeInfo findSendButton(AccessibilityNodeInfo node) {
        if (node == null) return null;
        String label = normalized(node.getText()) + " "
                + normalized(node.getContentDescription()) + " "
                + normalized(node.getViewIdResourceName());
        boolean looksLikeSend = label.contains("send")
                || label.contains("submit")
                || label.contains("arrow_upward")
                || label.contains("composer_send");
        if (looksLikeSend && node.isClickable()) return node;
        for (int index = node.getChildCount() - 1; index >= 0; index--) {
            AccessibilityNodeInfo found = findSendButton(node.getChild(index));
            if (found != null) return found;
        }
        return null;
    }
    ...
    private void updateAnswerCandidate(AccessibilityNodeInfo root) {
        ...
        if (best != null && !best.equals(candidateAnswer)) {
            ...
            handler.postDelayed(speakWhenStable, 2600);
        }
    }
    ...
    private final Runnable speakWhenStable = new Runnable() {
        ...
        public void run() {
            ...
            if (quietFor < 2400) {
                handler.postDelayed(this, 2400 - quietFor);
                return;
            }
    ...
    private void collect(AccessibilityNodeInfo node, Set<String> values) {
        ...
        for (int index = 0; index < node.getChildCount(); index++) collect(node.getChild(index), values);
    }
